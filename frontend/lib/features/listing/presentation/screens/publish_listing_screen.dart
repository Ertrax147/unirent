...
import '../providers/aviso_arriendo_provider.dart';
...
class _PublishListingScreenState extends ConsumerState<PublishListingScreen> {
  ...
  Future<void> _publishListing() async {
    ...
    try {
      ...
      final currentUserId = ref.read(authStateProvider).user?.id ?? 'admin';
      
      await repository.createAviso({
        'arrendadorId': currentUserId,
        'titulo': _titleController.text,
        'descripcion': _descriptionController.text,
        'precio': double.tryParse(_priceController.text) ?? 0.0,
        'ciudad': _addressController.text,
        ...
        'disponible': true,
        'cuposDisponibles': _selectedType == 'Compartida' ? 2 : 1,
      });
