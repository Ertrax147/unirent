...
class _ChatDetailScreenState extends ConsumerState<ChatDetailScreen> {
  ...
  void _sendMessage() {
    ...
    ref.read(chatMessagesActionProvider).sendMessage(widget.chatId, text);
  }

  @override
  Widget build(BuildContext context) {
    final messagesState = ref.watch(chatMessagesStreamProvider(widget.chatId));
    ...
    if (chatsState.value != null) {
      try {
        currentChat = chatsState.value!.firstWhere((c) => c.id == widget.chatId);
        otherUserId = currentChat.studentId == myUserId ? currentChat.landlordId : currentChat.studentId;
      } catch (_) {}
    }
    
    if (currentChat != null) {
      final listingAsync = ref.watch(avisoDetailProvider(currentChat.listingId));
      if (listingAsync.value != null) {
        propertyTitle = listingAsync.value!.titulo;
      } else {
        propertyTitle = 'Propiedad #${currentChat.listingId}';
      }
    }
          ...
          IconButton(
            ...
            onPressed: () {
              if (otherUserId.isNotEmpty) {
                final listingId = currentChat?.listingId ?? '0';
                context.push('/listing/$listingId/reviews', extra: otherUserId);
              }
            },
          ),
          if (currentChat != null)
